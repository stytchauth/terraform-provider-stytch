...
func TestAccTrustedTokenProfileResource(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.TestName, func(t *testing.T) {
			...
			if tc.Initial.CanJITProvision {
				initialChecks = append(initialChecks, resource.TestCheckResourceAttr(resourceName, "can_jit_provision", "true"))
			} else {
				initialChecks = append(initialChecks, resource.TestCheckResourceAttr(resourceName, "can_jit_provision", "false"))
			}
			...
			if tc.Update.CanJITProvision {
				updateChecks = append(updateChecks, resource.TestCheckResourceAttr(resourceName, "can_jit_provision", "true"))
			} else {
				updateChecks = append(updateChecks, resource.TestCheckResourceAttr(resourceName, "can_jit_provision", "false"))
			}
